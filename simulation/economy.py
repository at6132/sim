...
class Resource:
    ...
    )  # Custom properties for emergent resource types
...
class Exchange:
    ...
    rules: Dict[str, Any] = field(
        default_factory=dict
    )  # Custom rules for this exchange
...
class Trade:
    ...
    properties: Dict[str, Any] = field(
        default_factory=dict
    )  # Custom properties for emergent trade types
...
class Market:
    ...
    created_at: float = field(default_factory=time.time)
    last_trade_time: Optional[float] = None
    price_history: List[Tuple[float, float]] = field(
...
class Portfolio:
    ...
    resources: Dict[str, float] = field(
        default_factory=dict
    )  # Can hold any resource type
    created_at: float = field(default_factory=time.time)
    last_updated: float = field(default_factory=time.time)
    properties: Dict[str, Any] = field(
        default_factory=dict
    )  # Custom properties for emergent portfolio types
...
class EconomicSystem:
    ...
    def create_exchange(
        self, name: str, description: str, type: str, rules: Dict[str, Any] = None
    ) -> Exchange:
        ...
        exchange = Exchange(
            name=name, description=description, type=type, rules=rules or {}
        )
    ...
    def create_portfolio(
        self, owner: str, properties: Dict[str, Any] = None
    ) -> Portfolio:
        ...
        portfolio = Portfolio(owner=owner, properties=properties or {})
    ...
    def execute_trade(
        ...
        for participant, goods in zip(
            participants[: len(goods_given)], goods_given.items()
        ):
            ...
            if (
                goods[0] not in self.portfolios[participant].resources
                or self.portfolios[participant].resources[goods[0]] < goods[1]
            ):
                logger.error(
                    f"Participant {participant} does not have enough {goods[0]}"
                )
                return False
        ...
        for participant, goods in zip(
            participants[len(goods_given) :], goods_received.items()
        ):
            if participant not in self.portfolios:
    ...
    def _update_exchanges(self, time_delta: float):
        ...
        for exchange in self.exchanges.values():
            # Decay volume slightly over time
            exchange.volume = max(0.0, exchange.volume * (1 - 0.01 * time_delta))

            # Small random fluctuation in efficiency
            change = random.uniform(-0.005, 0.005) * time_delta
            exchange.efficiency = min(1.0, max(0.5, exchange.efficiency + change))

    def _update_portfolios(self, time_delta: float):
        """Update portfolio states based on emergent rules."""
        for portfolio in self.portfolios.values():
            for resource, amount in list(portfolio.resources.items()):
                portfolio.resources[resource] = max(
                    0.0, amount * (1 - 0.005 * time_delta)
                )
    ...
    def _check_economic_events(self, time_delta: float):
        """Check for emergent economic events."""
        for exchange in self.exchanges.values():
            if exchange.efficiency < 0.6:
                logger.warning(f"Exchange {exchange.name} efficiency dropping")

        for owner, portfolio in self.portfolios.items():
            if sum(portfolio.resources.values()) < 1:
                logger.info(f"Portfolio for {owner} is nearly empty")
